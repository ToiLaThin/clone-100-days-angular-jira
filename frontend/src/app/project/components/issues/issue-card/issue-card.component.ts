...
import { selectorIssueOfId, selectorUsers } from '../../../../state/project/project.selectors';
...
export class IssueCardComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.usersSubscription = this._store
            .select((state) => selectorUsers(state as { [projectFeatureKey]: IProjectState }))
            .subscribe((allUsers) => {
    ...
    openIssueModal(issueId: string) {
        this._modalService.create({
            ...
            nzData: {
                ...
                issueSelected$: this._store.select((state) =>
                    selectorIssueOfId(issueId)(state as { [projectFeatureKey]: IProjectState })
                )
            }
        });
    }
